...
public class GameConfig : ScriptableObject
{
    ...
    private void OnValidate()
    {
        ...
        // Validar valores de resistência
        maxPhysicalResistance = Mathf.Clamp(maxPhysicalResistance, 0.5f, 0.9f);
        maxElementalResistance = Mathf.Clamp(maxElementalResistance, 0.5f, 0.9f);

        // Validar valores de chance crítica
        baseCriticalChance = Mathf.Clamp(baseCriticalChance, 0f, 0.2f);
        criticalChancePerDexterity = Mathf.Clamp(criticalChancePerDexterity, 0.001f, 0.01f);

        // Validar multiplicadores
        baseCriticalMultiplier = Mathf.Clamp(baseCriticalMultiplier, 1f, 3f);
        criticalMultiplierPerDexterity = Mathf.Clamp(criticalMultiplierPerDexterity, 0.001f, 0.05f);

        // Validar velocidades
        baseAttackSpeed = Mathf.Max(0.1f, baseAttackSpeed);
        baseCastSpeed = Mathf.Max(0.1f, baseCastSpeed);
        attackSpeedPerDexterity = Mathf.Clamp(attackSpeedPerDexterity, 0.001f, 0.02f);
        castSpeedPerIntelligence = Mathf.Clamp(castSpeedPerIntelligence, 0.001f, 0.02f);

        // Validar redução de mana
        maxManaReduction = Mathf.Clamp(maxManaReduction, 0.1f, 0.5f);
        manaReductionPerIntelligence = Mathf.Clamp(manaReductionPerIntelligence, 0.0001f, 0.005f);

        // Log de validação
        if (showDebugGizmos)
        {
            UnityEngine.Debug.Log("[GameConfig] Validação concluída com sucesso");
        }
    }
    ...
    private void OnEnable()
    {
        if (showDebugGizmos)
        {
            UnityEngine.Debug.Log("[GameConfig] Configuração carregada");
        }
    }
